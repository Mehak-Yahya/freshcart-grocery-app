...
const ResetPassword = () => {
  ...
  const handleVerifyCode = async (e) => {
    ...
    if (!email) {
      setCodeMessage(
        "No email available. Please request a reset link again from Forgot Password.",
      );
      setCodeError(true);
      return;
    }
    ...
    if (savedCode) {
      ...
      if (savedCode === code.trim()) {
        ...
        setCodeMessage(
          "Code incorrect. Please check the code sent to your email.",
        );
        setCodeError(true);
      }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCodeMessage(
          data.message || "Code verified. You can set a new password now.",
        );
        setCodeError(false);
      } else {
        setCodeMessage(
      ...
      <div className="reset-password-form">
        ...
        {!tokenState && (
          <form onSubmit={handleVerifyCode}>
            ...
            {codeMessage && (
              <div
                className={`reset-message ${codeError ? "error" : "success"}`}
              >
                {codeMessage}
              </div>
            )}
